...
// function prototypes
void bike(int, int);
int get_box(float theta, float theta_dot, float omega, float omega_dot, float omega_d_dot, float psi_goal);
...
int main(void)
{
	...
	int action = 1;
	int to_do = initialize;
	int i = 0;

    bike(action, initialize);	
	
	while(action != -1)
	{
		printf("\n\nEnter an action (int, -1 to exit):");
		scanf("%d",&action);
		//bike(return_state, return_state2, action, reinforcement, initialize);
		bike(action, execute_action);
	}
	
}

void bike(int action, int to_do)
{
  static float reinforcement;
  ...
  T = (float) 2*((action / 3)-1);
  d = (float) 0.02*((action % 3)-1);
 //d = (float) (d + 0.04*(0.5-random)); /* Max noise is 2 cm */
  ...
  case execute_action : {
	...
	printf("\t dt = %3.3f\n", dt);
	printf("\t [theta, theta_dot, omega, omega_dot]\n");
	printf("\t [%3.3f, %3.3f, %3.3f, %3.3f]\n", theta, theta_dot, omega, omega_dot);
...
float calc_dist_to_goal(float xf, float xb, float yf, float yb)
{
  ...
  temp = (float) sqrt(max(0, (x_goal-xf)*(x_goal-xf) + (y_goal-yf)*(y_goal-yf) 
